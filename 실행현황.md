...
#### 36. 실행현황.md 기록 강제 훅 설정
- [x] `auto-record.sh` 업데이트: 파일 수정 시 대기 상태 저장
- [x] `check-record.sh` 신규 생성: 실행현황.md 업데이트 전까지 다음 파일 수정 차단
- [x] `settings.json`에 PreToolUse 훅 추가 (Edit|Write → check-record.sh)
- [x] 흐름: 파일 수정 → 알림 → 다음 수정 차단 → 실행현황.md 업데이트 → 차단 해제
...
#### 36. 조합형 옵션으로 변경
- [x] 단독형 옵션(optionSimple) → 조합형 옵션(optionCombinations)으로 변경
- [x] `generateOptionCombinations` 함수 추가: 사이즈 x 색상 조합 자동 생성

#### 37. 사이즈/컬러 입력 동기화
- [x] ProductForm 입력 → SalesInfo 자동 반영
- [x] 문구 생성 + 등록에 같은 데이터 사용
